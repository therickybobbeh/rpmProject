...
class Agent:
    ...
    def Solve(self, problem):
        ...
        if a_to_b is not None:
            ...
            a_to_c = self.find_match_1_and_2(node_a, self.images_data.get("C"), self.images_data.get("B"))
    ...
    def preform_action_list(self, action_list_to_preform, image_to_preform_transformations_on):
        image = image_to_preform_transformations_on
        for action_name in action_list_to_preform:
            for available_action_name, action_fn in self.actions:
                if np.array_equal(action_name, available_action_name):
        ...
        #         if np.array_equal(filter_name, available_filter_name):
